...
- Continue frontend phase-2 hardening after phase-1 modularization: split `useWorkflowDetailController` into focused sub-hooks/modules and keep reducing large client surfaces (`GalleryWorkspace`, `ImageModal`, `TopBar`) with behavior-preserving extractions.
