...
function prepareStatements(database) {
  return {
    ...
    selectWorkflows: database.prepare(
      'SELECT id, name, description, api_json, folder_id, sort_order, created_at, updated_at FROM workflows ORDER BY (folder_id IS NOT NULL), folder_id ASC, sort_order ASC, name ASC'
    ),
    selectWorkflowById: database.prepare(
      'SELECT id, name, description, api_json, folder_id, sort_order, created_at, updated_at FROM workflows WHERE id = ?'
    ),
    selectWorkflowsByFolder: database.prepare(
      'SELECT id, name, description, api_json, folder_id, sort_order, created_at, updated_at FROM workflows WHERE folder_id = ? ORDER BY sort_order ASC, name ASC'
    ),
    selectWorkflowsWithoutFolder: database.prepare(
      'SELECT id, name, description, api_json, folder_id, sort_order, created_at, updated_at FROM workflows WHERE folder_id IS NULL ORDER BY sort_order ASC, name ASC'
    ),
